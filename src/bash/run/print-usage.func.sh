...
EOF_USAGE_02
  while read -r step; do
    ...
    while read -r app; do
      for env in $(echo dev tst stg prd all); do echo ORG=spe APP=$app ENV=$env make do-generate-conf-for-$step; done
    done < <(
      cat <<EOF_APPS
nba
prp
...
EOF_APPS
  ...
  done < <(ls -1 /opt/spe/spe-prp-infra/src/terraform/ | grep -v remote-bucket | sort | grep -v modules)
  exit 1
}
